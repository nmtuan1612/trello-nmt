...
export const boardSlice = createSlice({
              ...
              columns: [
                {
                  ...
                  cards: [
                    { id: "card-1", boardId: "board-1", columnId: "column-1", title: "Title of card 1", cover: "https://scontent.fhan14-1.fna.fbcdn.net/v/t45.1600-4/277895101_6292993313882_1968459955091717650_n.jpg?stp=cp0_dst-jpg_p180x540_q75_spS444&_nc_cat=107&ccb=1-5&_nc_sid=67cdda&_nc_ohc=peTOX7IllF0AX-wc6ov&_nc_ht=scontent.fhan14-1.fna&oh=00_AT8QmQNCN3S_7LV7W1e915MiUFFWlQyTKB0-5le9ESLtpA&oe=62664F45",},
            ...
            },
            {
              ...
              columns: [
                ...
                },
                {
                    ...
                    cardOrder: ["card-8", "card-9", "card-10"],
                    cards: [
                      { id: "card-8", boardId: "board-2", columnId: "column-2", title: "Title of card 8", cover: null },
                      { id: "card-9", boardId: "board-2", columnId: "column-2", title: "Title of card 9", cover: null },
                      { id: "card-10", boardId: "board-2", columnId: "column-2", title: "Title of card 10", cover: null },
                    ],
                },
                {
                    id: "column-3",
                    ...
                    cardOrder: ["card-11", "card-12", "card-13"],
                    cards: [
                      { id: "card-11", boardId: "board-2", columnId: "column-3", title: "Title of card 11", cover: null },
                      { id: "card-12", boardId: "board-2", columnId: "column-3", title: "Title of card 12", cover: null },
                      { id: "card-13", boardId: "board-2", columnId: "column-3", title: "Title of card 13", cover: null },
                    ],
                },
              ],
            },
    ...
    reducers: {
        ...
        createColumn: (state, action) => {
            const currBoard = state.boards.find(board => board.id === action.payload.boardId)
            const idx = state.boards.indexOf(currBoard)
            state.boards[idx].columns = [...state.boards[idx].columns, action.payload]
        },
        addCard: (state, action) => {
            const currBoard = state.boards.find(board => board.id === action.payload.boardId)
            const idxBoard = state.boards.indexOf(currBoard)
            const currColumn = state.boards[idxBoard].columns.find(column => column.id === action.payload.columnId)
            const idxColumns = state.boards[idxBoard].columns.indexOf(currColumn)
            state.boards[idxBoard].columns[idxColumns].cards = [...state.boards[idxBoard].columns[idxColumns].cards, {...action.payload, id: `card-${state.boards[idxBoard].columns[idxColumns].cards.length + 1}`}]
        },
        ...
        editColumnTitle: (state, action) => {
            const currBoard = state.boards.find(board => board.id === action.payload.boardId)
            const idxBoard = state.boards.indexOf(currBoard)
            state.boards[idxBoard].columns[action.payload.columnPost].title = action.payload.title;
        },
        editCardTitle: (state, action) => {
            ...
            const currColumn = state.boards[idxBoard].columns.find(column => column.id === action.payload.columnId)
            const idxColumns = state.boards[idxBoard].columns.indexOf(currColumn)
            state.boards[idxBoard].columns[idxColumns].cards[action.payload.cardPost].title = action.payload.title
        },
...
export const { createBoard, createColumn, addCard, editColumnTitle, editBoardName, editCardTitle, starBoard, changeBoard } = boardSlice.actions;
